<x-app-layout>
    <div class="bg-gray-100">
        <form method="post" action="{{route('vacancy-put', ['vacancy' => $vacancy->id])}}">
            @method('PUT')
            @csrf
            <div class="flex flex-col mb-4">
                <div class="flex flex-col mb-4">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input type="text" value='{{$vacancy->title}}' name="title" id="title" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">
                    </div>
                    @error('title')
                ...
                <div class="flex flex-col mb-4">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input type="text" value='{{$vacancy->summary}}' name="summary" id="summary" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">
                    </div>
                    @error('summary')
                    <div class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">
                        {{ $message }}
                ...
                <div class="flex flex-col mb-4">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                         <textarea name="description" id="description" rows='4' class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">{{$vacancy->description}}</textarea>
                    </div>
                    @error('description')
                    <div class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">
                        {{ $message }}
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <select name="city_id" id="city" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">
                            @foreach ($cities as $city)
                                <option value="{{$city->id}}" {{$vacancy->city_id === $city->id ? 'selected' : ''}}>{{$city->name}} ({{$city->postal_code}})</option>
                            @endforeach
                        </select>
                    </div>
                    @error('city_id')
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <select name="company_id" id="company" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">
                            @foreach ($companies as $company)
                                <option value="{{$company->id}}" {{$vacancy->company_id === $company->id ? 'selected' : ''}}>{{$company->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('company_id')
